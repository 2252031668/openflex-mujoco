...
```bash
uv run  python3 viewer.py            # 带灵巧手 + 全面自碰撞（推荐，默认）
uv run  python3 viewer.py --plain    # 仅地面碰撞原版（无手）
uv run  python3 viewer.py --check    # 仅校验可加载
```
...
```bash
uv run python3 viewer.py            # 默认：带手 + 全面自碰撞（左手/右手各 20 关节 + 双臂/头/升降/转向，共 61 执行器）
uv run python3 viewer.py --check    # 仅校验可加载
uv run python3 viewer.py --plain        # 版本A：仅地面碰撞（无手）
uv run python3 viewer.py --self-collision  # 版本B：原夹爪 + 全身自碰撞（无手）
```
...
```bash
uv run python3 build_hand_xml.py     # 生成 openflex_mujoco_hand.xml
```
